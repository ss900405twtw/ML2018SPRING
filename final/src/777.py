...
def listdir_fullpath(d):
    ...
    return [ "./dataset/training_data/1_train.txt","./dataset/training_data/2_train.txt","./dataset/training_data/3_train.txt","./dataset/training_data/4_train.txt","./dataset/training_data/5_train.txt"] , train
...
def dual_encoder(sequence_length,embedding_matrix,input_dim):
    ...
    model.add(GRU(units=128,dropout=0.4 , return_sequences=False))
...
#word_dict = word2vec.Word2Vec.load("0614_embedding_test2_yuchi_0.48063.model")
word_dict = word2vec.Word2Vec.load("model.bin")
print("word2vec")
model=word_dict
#model.save('yuchi.bin')
'''
#word_dict = word2vec.Word2Vec.load("0614_embedding_test2_yuchi_0.48063.model")
#word_dict = word2vec.Word2Vec.load("model.bin")
print(model)
...
checkpoint = ModelCheckpoint("./earlystop_model.h5", monitor='loss', save_best_only=True, verbose=1, mode='min')
dual_encoder_model.fit([Q,R],L,validation_data=([Q_val,R_val],L_val), batch_size=1024, epochs=100, verbose=1, callbacks=[checkpoint, earlystopping])
'''
model = load_model("./777.h5")
