...
model2=Word2Vec.load('0621.bin')
model=Word2Vec.load('0623_spacewindow15.bin')
#model3=Word2Vec.load('0623_spacewindow25.bin')
model3=Word2Vec.load('0623_spacewindow25.bin')
...
def weighted2(vocab):
	freq=model.wv.vocab[vocab].count/(summ2)
	a=0.0025
	weight=a/(a+freq)
	#weight=1/((freq)**0.5)
	return np.array(model[vocab])*weight
 
def weighted3(vocab):
	freq=model3.wv.vocab[vocab].count/(summ2)
	a=0.0025
	weight=a/(a+freq)
	#weight=1/((freq)**0.5)
	return np.array(model3[vocab])*weight
...
for i in range(len(jieba_6)):
    ...
    for j in range(len(jieba_6[i])):
		vec=vec+1/3*(weighted(jieba_6[i][j])+weighted2(jieba_6[i][j])+weighted3(jieba_6[i][j]))
    #print(type(vec))
    #print(len(jieba_7[i]))
...
for i in range(len(jieba_7)):
	...
	for j in range(len(jieba_7[i])):
		vec=vec+1/3*(weighted(jieba_7[i][j])+weighted2(jieba_7[i][j])+weighted3(jieba_7[i][j]))
	#print(type(vec))
	#print(len(jieba_7[i]))
